...
mod tests {
    ...
    fn test_scan_config_creation() {
        ...
        assert!(!config.rotate_ip_headers);
    ...
    fn test_scan_state_creation() {
        ...
        assert_eq!(state.found_count.load(Ordering::Relaxed), 0);
        assert_eq!(state.error_count.load(Ordering::Relaxed), 0);
        assert_eq!(state.filtered_count.load(Ordering::Relaxed), 0);
        assert!(!state.should_stop.load(Ordering::Relaxed));
    }

    #[test]
    fn test_scan_state_atomic_operations() {
        let state = create_test_state();

        // Test atomic increments
        state.found_count.fetch_add(1, Ordering::Relaxed);
        state.error_count.fetch_add(2, Ordering::Relaxed);
        state.filtered_count.fetch_add(3, Ordering::Relaxed);
        state.global_delay.fetch_add(100, Ordering::Relaxed);

        assert_eq!(state.found_count.load(Ordering::Relaxed), 1);
        assert_eq!(state.error_count.load(Ordering::Relaxed), 2);
        assert_eq!(state.filtered_count.load(Ordering::Relaxed), 3);
        assert_eq!(state.global_delay.load(Ordering::Relaxed), 100);
    ...
    fn test_wildcard_profile_performance() {
        ...
        for i in 0..1000 {
            let html_body = format!(
                r#"<html><head><title>Page {}</title></head><body>Content {}</body></html>"#,
                i, i
            );
            let sample = WildcardSample::from_response(&html_body, 404, &headers);
            profile.add_sample(&sample);
        ...
        assert!(profile.size_ranges.len() > 0);
    }
